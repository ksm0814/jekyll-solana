...
여기서 참 좋다고 느꼈던건, addParameter 메소드.
...
1. pivot(값)과 left, right(위치)를 저장<br/>

2. left에 있는 값이 pivot보다 클때까지 위치를 이동, right 값을 pivot보다 작을때까지 위치를 이동<br/>

3. 아직 left, right 위치가 바뀌지 않았다면 왼쪽 오른쪽 값을 바꿔준다. 그리고 1번으로 돌아감<br/>

4. left, right가 서로 바뀐거라면 양쪽 부분에 대해 left, right, pivot을 각각 정해주고 재귀<br/>
